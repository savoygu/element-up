...
:::tip
组件书写形式发生变化：`<el-tree>`修改为`<up-tree>`
:::
...
:::demo
```html
<up-tree :data="data" :props="defaultProps" @node-click="handleNodeClick"></up-tree>
...
<div class="buttons">
  <up-button @click="getCheckedNodes">通过 node 获取</up-button>
  <up-button @click="getCheckedKeys">通过 key 获取</up-button>
  <up-button @click="setCheckedNodes">通过 node 设置</up-button>
  <up-button @click="setCheckedKeys">通过 key 设置</up-button>
  <up-button @click="resetChecked">清空</up-button>
</div>
      ...
      renderContent(h, { node, data, store }) {
        return (
          <span class="custom-tree-node">
            <span>{node.label}</span>
            <span>
              <up-button size="mini" type="text" on-click={ () => this.append(data) }>Append</up-button>
              <up-button size="mini" type="text" on-click={ () => this.remove(node, data) }>Delete</up-button>
            </span>
          </span>);
      }
    }
