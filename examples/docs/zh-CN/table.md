...
:::tip
 ...
 1. 组件书写形式发生变化：`<el-table>`修改为`<up-table>`，`<el-table-column>`修改为`<up-table-column>`
 2. 封装了`page`对象，简化了`up-table`的编写
 3. 支持不同形式的表格提供了自定义slot和新增了props
:::
...
```html
<template>
  <up-table :page="page" style="width: 100%"></up-table>
  <!-- <el-table
    :data="tableData"
    style="width: 100%">
    <el-table-column
...
```html
<template>
  <up-table :page="page" stripe style="width: 100%"></up-table>
  <!-- <el-table
    :data="tableData"
    stripe
    style="width: 100%">
...
```html
<template>
  <up-table :page="page" border style="width: 100%"></up-table>
  <!-- <el-table
    :data="tableData"
    border
    style="width: 100%">
...
```html
<template>
  <up-table :page="page" border style="width: 100%" :row-class-name="tableRowClassName"></up-table>
  <!-- <el-table
    :data="tableData"
    style="width: 100%"
...
```html
<template>
  <up-table :page="page" height="250" border style="width: 100%"></up-table>
  <!-- <el-table
    :data="tableData"
    height="250"
    border
...
<template>
  <up-table
    :page="page"
    :fixed-columns="{ date: 'left' }"
    :width-columns="{ date: 150, name: 120, province: 120, city: 120, address: 300, zip: 120 }"
    border
    style="width: 100%">
    <template slot="up:operate">
      <up-table-column
        fixed="right"
        label="操作"
        width="100">
        <template slot-scope="scope">
          <up-button @click="handleClick(scope.row)" type="text" size="small">查看</up-button>
          <up-button type="text" size="small">编辑</up-button>
        </template>
      </up-table-column>
    </template>
  </up-table>
...
<template>
  <up-table
    ...
    <up-table-column label="配送信息">
      <up-table-column
        prop="name"
        label="姓名"
        width="120">
      </up-table-column>
      <up-table-column label="地址">
        <up-table-column
          prop="province"
          label="省份"
          width="120">
...
<template>
  ...
  <div style="margin-top: 20px">
    <up-button @click="setCurrent(page.data[1])">选中第二行</up-button>
    <up-button @click="setCurrent()">取消选择</up-button>
  </div>
  <!-- <el-table
    ref="singleTable"
    :data="tableData"
  ...
  <div style="margin-top: 20px">
    <up-button @click="setCurrent(tableData[1])">选中第二行</up-button>
    <up-button @click="setCurrent()">取消选择</up-button>
  </div> -->
</template>
...
<template>
  ...
  <div style="margin-top: 20px">
    <up-button @click="toggleSelection([page.data[1], page.data[2]])">切换第二、第三行的选中状态</up-button>
    <up-button @click="toggleSelection()">取消选择</up-button>
  </div>
  <!-- <el-table
    ref="multipleTable"
    :data="tableData"
...
<template>
  <up-table
    ...
    :sort-columns="['date', 'name']"
    :slot-columns="['address']"
    :default-sort="{prop: 'date', order: 'descending'}"
    >
    <up-table-column
      slot="address"
      slot-scope="{value, key}"
      :prop="key"
      :label="value"
      :formatter="formatter">
    </up-table-column>
  ...
  export default {
    ...
    methods: {
      formatter(row, column) {
        return row.address + ' by2 ' + row.name;
      }
    }
  }
</script>
...
```html
<template>
  <up-button @click="resetDateFilter">清除日期过滤器</up-button>
  <up-button @click="clearFilter">清除所有过滤器</up-button>
  <up-table
    ref="filterTable"
    :page="page"
    style="width: 100%"
    :width-columns="{ date: 180, name: 180, tag: 100 }"
    :sort-columns="['date']"
    :slot-columns="['address']"
    :slot-contents="['tag']"
    :options="options"
  >
    ...
    <template slot="tag:content" slot-scope="scope">
      <up-tag
        :type="scope.row.tag === '家' ? 'primary' : 'success'"
        disable-transitions>{{scope.row.tag}}</up-tag>
    </template>
  </up-table>
  <!--
...
<template>
  <up-table
    ...
    style="width: 100%"
    :width-columns="{ date: 180, name: 180 }"
    :slot-contents="['date', 'name']"
    >
    <template slot="date:content" slot-scope="scope">
      <i class="el-icon-time"></i>
      <span style="margin-left: 10px">{{ scope.row.date }}</span>
    </template>
    <template slot="name:content" slot-scope="scope">
      <up-popover trigger="hover" placement="top">
        <p>姓名: {{ scope.row.name }}</p>
        <p>住址: {{ scope.row.address }}</p>
        <div slot="reference" class="name-wrapper">
          <up-tag size="medium">{{ scope.row.name }}</up-tag>
        </div>
      </up-popover>
    </template>
    <up-table-column slot="up:operate" label="操作">
      <template slot-scope="scope">
        <up-button
          size="mini"
          @click="handleEdit(scope.$index, scope.row)">编辑</up-button>
        <up-button
          size="mini"
          type="danger"
          @click="handleDelete(scope.$index, scope.row)">删除</up-button>
      </template>
    </up-table-column>
  </up-table>
  <!-- <el-table
...
<template>
  <up-table
    ...
    expand
    style="width: 100%">
    <up-table-column type="expand" slot="up:expand">
      <template slot-scope="props">
        <up-form label-position="left" inline class="demo-table-expand">
          <up-form-item label="商品名称">
            <span>{{ props.row.name }}</span>
          </up-form-item>
          <up-form-item label="所属店铺">
            <span>{{ props.row.shop }}</span>
          </up-form-item>
          <up-form-item label="商品 ID">
            <span>{{ props.row.id }}</span>
          </up-form-item>
          <up-form-item label="店铺 ID">
            <span>{{ props.row.shopId }}</span>
          </up-form-item>
          <up-form-item label="商品分类">
            <span>{{ props.row.category }}</span>
          </up-form-item>
          <up-form-item label="店铺地址">
            <span>{{ props.row.address }}</span>
          </up-form-item>
          <up-form-item label="商品描述">
...
<template>
  <up-table
    :page="{ ...page, data: page.data.filter(data => !search || data.name.includes(search)) }"
    style="width: 100%"
  >
    <up-table-column slot="up:operate" align="right">
      ...
      <template slot-scope="scope">
        <up-button
          size="mini"
          @click="handleEdit(scope.$index, scope.row)">Edit</up-button>
        <up-button
          size="mini"
          type="danger"
          @click="handleDelete(scope.$index, scope.row)">Delete</up-button>
      </template>
    </up-table-column>
  </up-table>
  <!-- <el-table
...
<template>
  <up-table
    ...
    show-summary
    style="width: 100%"
    :width-columns="{ id: 180 }"
    :sort-columns="['amount1', 'amount2', 'amount3']"
  >
  </up-table>
  <!-- <el-table
    :data="tableData"
...
| --------------- | ------------------------------------ | ------------------------- | --------------------- | ------ |
| page             | 显示的内容                           | object                     | —                     | —      |
| expand           | 是否支持展开                         | boolean                     | —                    | —      |
| selection        | 是否支持多选                         | boolean                     | —                    | false  |
| layout           | 分页布局方式                         | string                      | —                    | total,sizes,prev,pager,next,jumper  |
| pagination       | 是否支持分页                         | boolean                     | —                    | false      |
| fixedColumns     | 要固定的列                           | object                      |  —                   | {}      |
| minWidthColumns  | 设置最小宽度                         | object                      | —                    | {}     |
| widthColumns     | 设置宽度                             | object                     | —                     | {}     |
| slotColumns     | 设置插槽列                             | array                     | —                     | []     |
| slotContents     | 设置插槽列内容                         nml| array                     | —                     | []     |
